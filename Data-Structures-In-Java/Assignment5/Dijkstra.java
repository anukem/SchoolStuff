...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        ...
        double verticalDistance = uy - vy;
        double horizontalDistance = ux - vx;
        double sumOfSquares = (horizontalDistance*horizontalDistance) + (verticalDistance*verticalDistance);
   ...
   */
  public void computeAllEuclideanDistances() throws FileNotFoundException {
        // TODO
        String data = "citypairs.txt";
        ...
        while(in.hasNextLine()){
          ...
          int ux = getVertex(pairs[0]).x;
          int uy = getVertex(pairs[0]).y;
          int vx = getVertex(pairs[1]).x;
          int vy = getVertex(pairs[1]).y;
          double newWeight = computeEuclideanDistance(ux,uy,vx,vy);

          for (Edge edge: getVertex(pairs[0]).adjacentEdges) {
            
            if (edge.target.equals(getVertex(pairs[1]))) {
              edge.distance = newWeight;

            }
          }

          for (Edge edge: getVertex(pairs[1]).adjacentEdges) {
            
            if (edge.target.equals(getVertex(pairs[0]))) {
              edge.distance = newWeight;
...
public Vertex smallestUnknownVertex() {
          ...
          Vertex smallestUnknownVertex = new Vertex("biggest", 2, 4);
          smallestUnknownVertex.distance = Double.POSITIVE_INFINITY;

          for (Vertex v: getVertices() ) {
              if(!v.known && v.distance < smallestUnknownVertex.distance) {
                  smallestUnknownVertex = v;
              }

          }
          System.out.println(smallestUnknownVertex);
          return smallestUnknownVertex;
}
  ...
  public void doDijkstra(String s) {
      ...
      Vertex startingPoint = getVertex(s);
      startingPoint.distance = 0;
      Vertex current = startingPoint;
          ...
          for (Edge edge:current.adjacentEdges) {
             ...
             Vertex adjacentVertex = edge.target;
              ...
              if (!adjacentVertex.known) {
                  ...
                  if (current.distance + cvw < adjacentVertex.distance) {
                      
                      adjacentVertex.distance = current.distance + cvw;
                      adjacentVertex.prev = current;
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    LinkedList<Edge> dijkstraPath = new LinkedList<Edge>();
    ...
    return dijkstraPath; // Replace this
  }
